...
Operator Call spans are nested under their NodeExecution in the Timeline and use the Call's recorded start and end. The projection keeps at most 50 Call rows per NodeExecution and preserves exact aggregate counts plus an omitted count; the Runtime Event journal remains complete and paged. Selecting a Call or Event loads its canonical input/output from Event detail instead of copying values into the Timeline projection. Database ArtifactRefs remain small until the user explicitly loads their Invocation-owned JSON value; repeated references share the UI query cache.
