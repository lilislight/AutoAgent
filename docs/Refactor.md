...
但不能作为新结构的模板照搬；`v1/docs-deprecated/` 也不是本次重构的设计来源。

当前仓库根目录就是 V2 项目；每个边界以本文件的契约和根目录 V2 测试共同验证。
完整 V1 项目归档在 `v1/`，只用于历史参考，不参与 V2 导入、构建或测试。
...
class Invocation:
    ...
    def done(self) -> bool: ...
    def join(self, timeout: float | None = None) -> Invocation: ...
    async def ajoin(self, timeout: float | None = None) -> Invocation: ...
...
- 调用者可以用 `join()` / `ajoin()` 等待其他尚未收敛的取消清理完成。
