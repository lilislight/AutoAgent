...
from autoagent.core.runtime.sinks import RuntimeEventSink
from autoagent.core.runtime.store import RuntimeStore, SessionBusyError
...
class SQLiteRuntimeStore(RuntimeStore):
    ...
    ``initialize`` creates the current V1 schema for a new database. AutoAgent
    is not published yet, so V1 deliberately does not upgrade or preserve older
    ...
    async def initialize(self) -> None:
        """Create the current schema once; safe to call before every App startup."""
