...
class OutputContext:
    ...
    def __init__(self, outputs: list[NodeOutput] | None = None) -> None:
    ...
    def has(self, node_id: str) -> bool:
        return bool(self._by_node_id.get(node_id))

    def latest(self, node_id: str, default: Any = _MISSING) -> Any:
        outputs = self._by_node_id.get(node_id)
        if outputs:
    ...
    def all(self, node_id: str) -> list[Any]:
        return [output.value for output in self._by_node_id.get(node_id, [])]
